...
$start = ($_GET['id'] - 1) * 50;
$end = $_GET['id']  * 50;
$sqlTotal = "SELECT count(1) AS `count` FROM `products` WHERE `prod_id`>$start && `prod_id`<$end ;";
...
<!-- breadcrumb -->
<?php
$sqllink = "SELECT * FROM `lookbook` WHERE `lookbook_id`= {$_GET['id']}";
$stmt = $pdo->query($sqllink);
if ($stmt->rowCount() > 0) {
    /**
     * 如果查詢結果很多筆
     * $stmt->fetchAll()
     ...
     */
    $arr = $stmt->fetchAll();
    foreach ($arr as $objlink) {
        $name = $objlink['name'];
    }
}
?>
<div class="container breadcrumb">
    <a href="./home.php">首頁</a>/<a href="./features_fashion.php">Features</a>/<a href="./features_lookbook.php">Lookbook</a>/<a href="./features_lookbook_detail.php?id=<?= $_GET['id'] ?>"><?= $name ?></a>
</div>
<?php
...
<div class="container cards">
    <?php
    $sql = "SELECT * FROM `products` WHERE `prod_id`>$start && `prod_id`<$end  LIMIT {$offset}, {$numPerPage}";
    $stmt = $pdo->query($sql);
    if ($stmt->rowCount() > 0) {
        /**
         * 如果查詢結果很多筆
    ...
    <div class="product_pagination">
        ...
        ?>
                <li class="page-item <?= $strClass; ?>">
                    <a class="page-link" href="./features_lookbook_detail.php?id=<?= $_GET['id'] ?>&page=<?= $i ?>">
                        <?= $i ?>
                    </a>
                </li>
        <?php
