...
<?php session_start() ?>
<?php
$sqlTotal = "SELECT count(1) AS `count` FROM `products` WHERE `prod_id`>200 && `prod_id`<250;";
...
<div class="container breadcrumb">
    <a href="./home.php">首頁</a>/<a href="./features_fashion.php">Features</a>/<a href="./features_lookbook.php">Lookbook</a>/<a href="./features_lookbook_detail5.php">Lookbook內頁</a>
</div>
<!-- title_box -->
...
<div class="container cards">
    <?php
    $sql = "SELECT * FROM `products` WHERE `prod_id`>200 && `prod_id`<250 LIMIT {$offset}, {$numPerPage}";
    $stmt = $pdo->query($sql);
    if ($stmt->rowCount() > 0) {
        /**
         * 如果查詢結果很多筆
    ...
    <div class="product_pagination">
        ...
        ?>
                <li class="page-item <?= $strClass; ?>" data-page="<?= $i ?>">
                    <a class="page-link" href="features_lookbook_detail5.php?page=<?= $i ?>">
                        <?= $i ?>
                    </a>
                </li>
        <?php
